...
func (sq *ShareQuery) QueryPost() *PostQuery {
...
func (sq *ShareQuery) WithPost(opts ...func(*PostQuery)) *ShareQuery {
	query := (&PostClient{config: sq.config}).Query()
...
func (sq *ShareQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Share, error) {
	...
	if query := sq.withPost; query != nil {
		if err := sq.loadPost(ctx, query, nodes, nil,
			func(n *Share, e *Post) { n.Edges.Post = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *ShareQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Share, init func(*Share), assign func(*Share, *Post)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Share)
	for i := range nodes {
		if nodes[i].post_shares == nil {
			continue
		}
		fk := *nodes[i].post_shares
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_shares" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
