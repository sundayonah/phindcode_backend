...
func (pq *PostQuery) QueryShares() *ShareQuery {
	query := (&ShareClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withShares:   pq.withShares.Clone(),
...
func (pq *PostQuery) WithShares(opts ...func(*ShareQuery)) *PostQuery {
	query := (&ShareClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withShares; query != nil {
		if err := pq.loadShares(ctx, query, nodes,
			func(n *Post) { n.Edges.Shares = []*Share{} },
			func(n *Post, e *Share) { n.Edges.Shares = append(n.Edges.Shares, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PostQuery) loadShares(ctx context.Context, query *ShareQuery, nodes []*Post, init func(*Post), assign func(*Post, *Share)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.post_shares
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_shares" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_shares" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
